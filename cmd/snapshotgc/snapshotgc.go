...
func deleteSnapshots(
	...
	for _, snap := range snapshots {
		snap := snap
		// Remove finalizers from all integration pipelineruns associated with the
		// snapshot before deleting it, so the pipelineruns can be pruned gracefully
		integrationLogger := helpers.IntegrationLogger{Logger: logger}
		err := helpers.RemoveFinalizerFromAllIntegrationPipelineRunsOfSnapshot(
			context.Background(), cl, integrationLogger, snap, helpers.IntegrationPipelineRunFinalizer,
		)
		if err != nil {
			logger.Error(err, "Failed to remove finalizers from pipelineruns for snapshot.", "snapshot.name", snap.Name)
		}
