...
func deleteSnapshots(
	...
	for _, snap := range snapshots {
		snap := snap
		// Remove finalizers from all integration PLRs associated with this snapshot
		// before deleting it, so the PLRs can be pruned after the snapshot is gone.
		err := helpers.RemoveFinalizerFromAllIntegrationPipelineRunsOfSnapshot(
			context.Background(), cl, helpers.IntegrationLogger{Logger: logger},
			snap, helpers.IntegrationPipelineRunFinalizer,
		)
		if err != nil {
