...
func deleteSnapshots(
	...
	for _, snap := range snapshots {
		snap := snap
		// Remove finalizers from all integration pipelineruns of the snapshot
		// before deleting it, so the pipelineruns can be pruned gracefully
		...
		if err != nil {
			logger.Error(err, "Failed to remove finalizers from pipelineruns of snapshot.", "snapshot.name", snap.Name)
		}
