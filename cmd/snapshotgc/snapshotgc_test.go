...
var _ = Describe("Test garbage collection for snapshots", func() {
	...
	Describe("Test deleteSnapshots", func() {
		...
		It("Removes pipelinerun finalizers before deleting snapshots", func() {
			...
			snapsRemaining := &applicationapiv1alpha1.SnapshotList{}
			err := cl.List(context.Background(), snapsRemaining, &client.ListOptions{Namespace: "test-ns"})
			Expect(err).ShouldNot(HaveOccurred())
			Expect(snapsRemaining.Items).To(BeEmpty())

			// Verify pipelinerun finalizer was removed
			plrsRemaining := &tektonv1.PipelineRunList{}
			err = cl.List(context.Background(), plrsRemaining, &client.ListOptions{Namespace: "test-ns"})
			Expect(err).ShouldNot(HaveOccurred())
